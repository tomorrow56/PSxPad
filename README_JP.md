...
通信は1バイト単位で行います。
ホストはCMDへデータを送信しながら、同時にDATAから受信します。
コントローラは前のコマンドを処理した結果をホストに返します。

例えばボタン状態を取得する場合は、次のような通信シーケンスになります。

| CMD  |  CMDの内容   | DATA |   DATAの内容   |
|------|--------------|------|----------------|
| 0x01 | ID要求       | 0xFF | ダミー         |
| 0x42 | ボタン情報   | 0x** | ID             |
| 0xFF | クロック送信 | 0x5A | ACK            |
| 0xFF | クロック送信 | 0x** | Button Low     |
| 0xFF | クロック送信 | 0x** | Button High    |

### ボタンデータ

取得したボタン情報は、以下の順番で各ビットに割り当てられます。（LSB first)
ボタンが押されていない時は"1"、ボタンが押されている時は"0"になります。
